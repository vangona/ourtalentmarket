...
class NoticeregisterForm(forms.Form):
    title = forms.CharField(
        error_messages={"required": "제목을 입력해주세요."}, label="공지 제목")
    image = forms.ImageField(required=False)
    content = forms.CharField(
        error_messages={"required": "내용을 입력해주세요."}, label="공지 내용")
...
class IdAuthorizationForm(forms.Form):
    user_id = forms.CharField(
        error_messages={"required": "사용자 이름을 선택해주세요."}, label="사용자 이름"
    )
    authorization = forms.CharField(
        error_messages={"required": "승인여부를 확인해주세요."}, label="승인 여부"
    )
...
class NoticeMainForm(forms.Form):
    ...
    main = forms.CharField(
        error_messages={"required": "승인여부를 확인해주세요."}, label="메인 공지")
